  ...
  * `:broadcast` enables broadcast sending
  ...
  def arguments(options) do
    ...
    args = if local = Keyword.get(options, :local) do
      args = if address = Keyword.get(local, :address), do: [{ :ip, Socket.Address.parse(address) } | args], else: args

      if fd = Keyword.get(local, :fd), do: [{ :fd, fd } | args], else: args
    end
    ...
    args = if Keyword.has_key?(options, :broadcast), do: [{ :broadcast, Keyword.get(options, :broadcast) } | args], else: args

    args = if multicast = Keyword.get(options, :multicast) do
      args = if address = Keyword.get(multicast, :address), do: [{ :multicast_if, address } | args], else: args
      args = if loop = Keyword.get(multicast, :loop), do: [{ :multicast_loop, loop } | args], else: args

      if ttl = Keyword.get(multicast, :ttl), do: [{ :multicast_ttl, ttl } | args], else: args
    else
      args
    end

    if membership = Keyword.get(options, :membership), do: [{ :add_membership, membership } | args], else: args
  end
end
